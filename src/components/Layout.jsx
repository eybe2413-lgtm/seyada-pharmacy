    ...
    <div className="min-h-screen flex bg-bg">
      ...
      >
        ...
        <div className="p-3 border-t border-line">
          ...
          <button onClick={() => setLogoutConfirm(true)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold text-danger hover:bg-danger-soft">
            <LogOut size={18} />
            {t('common.logout')}
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-line flex items-center justify-between px-4 md:px-6 sticky top-0 z-30">
          <button className="md:hidden text-ink" onClick={() => setDrawerOpen(true)}>
            <Menu size={22} />
          </button>
      ...
      {/* التعديل ٧: نافذة تأكيد تسجيل الخروج */}
      {logoutConfirm && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-xs text-center">
            ...
            <div className="flex gap-3">
              <button onClick={() => setLogoutConfirm(false)} className="flex-1 py-2.5 rounded-xl border border-line text-sm font-bold text-ink hover:bg-bg">
                {t('common.no')}
              </button>
              <button onClick={handleLogout} className="flex-1 py-2.5 rounded-xl bg-danger text-white text-sm font-bold hover:bg-danger/90">
                {t('common.yes')}
              </button>
            </div>
          </div>
