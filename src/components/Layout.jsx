...
const NAV_ITEMS = [
  ...
  { to: '/accounts',         icon: Landmark,        key: 'nav.accounts',         managerOnly: true },
...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen flex bg-bg">
      ...
      <aside className="shrink-0 w-64 bg-white border-s border-line flex flex-col h-screen sticky top-0 overflow-y-auto">
        ...
        <div className="p-3 border-t border-line shrink-0">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold text-danger hover:bg-danger-soft"
          >
            <LogOut size={18} />
            {t('common.logout')}
          </button>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-line flex items-center justify-between px-4 sticky top-0 z-30">
          <div />
            ...
            <div className="relative">
              ...
              {notifOpen && (
                <div className="absolute end-0 mt-2 w-72 bg-white border border-line rounded-xl shadow-xl p-3 z-50">
                  ...
                  {notifCount === 0 && (
                    <div className="text-xs text-sub px-1 py-3 text-center">{t('notifications.noNotifications')}</div>
                  )}
                  {notifications.lowStock > 0 && (
                    <NotifRow color="bg-accent" text={`${notifications.lowStock} — ${t('notifications.lowStock')}`} onClick={() => { setNotifOpen(false); navigate('/medicines'); }} />
                  )}
                  {notifications.expiring > 0 && (
                    <NotifRow color="bg-danger" text={`${notifications.expiring} — ${t('notifications.expiry')}`} onClick={() => { setNotifOpen(false); navigate('/medicines'); }} />
                  )}
                  {isManager && notifications.debt > 0 && (
                    <NotifRow color="bg-primary" text={t('notifications.debtDue')} onClick={() => { setNotifOpen(false); navigate('/debts'); }} />
                  )}
                </div>
              )}
            </div>
      ...
      {logoutConfirm && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-xs text-center">
            ...
            <p className="font-bold text-ink text-base mb-5">{t('common.logoutConfirm')}</p>
            <div className="flex gap-3">
              <button
                onClick={() => setLogoutConfirm(false)}
                className="flex-1 py-2.5 rounded-xl border border-line text-sm font-bold text-ink hover:bg-bg"
              >
                {t('common.no')}
              </button>
              <button
                onClick={handleLogout}
                className="flex-1 py-2.5 rounded-xl bg-danger text-white text-sm font-bold hover:bg-danger/90"
              >
                {t('common.yes')}
              </button>
            </div>
